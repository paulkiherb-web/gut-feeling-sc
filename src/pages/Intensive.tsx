...
const DEPTH_LABELS: Record<Depth, { label: string; desc: string }> = {
  soft: { label: 'Мягкий', desc: 'Лёгкие привычки' },
  balanced: { label: 'Собранный', desc: 'Оптимальный баланс' },
  intense: { label: 'Фокус', desc: 'Максимум дисциплины' },
};
...
const PROTOCOLS: Record<string, ProtocolItem[]> = {
  energy: [
    ...
    { id: '4', time: '08:00', title: 'Omega-3 + D3', description: 'Базовые добавки', phase: 'morning', depths: ['balanced', 'intense'], newsSignal: 'D3 усваивается с жирами' },
  ...
  recovery: [
    ...
    { id: '5', time: '15:00', title: 'Вит. C + Цинк', description: 'Заживление', phase: 'day', depths: ['balanced', 'intense'] },
  ...
  sleep: [
    ...
    { id: '8', time: '21:00', title: 'Магний + глицин', description: 'Расслабление', phase: 'evening', depths: ['balanced', 'intense'] },
...
export default function Intensive() {
  ...
  return (
    <MobileLayout
      title="Интенсив"
      subtitle={`${goal?.icon} ${goal?.label}`}
      variant="warm"
      headerRight={
        <span className="px-2.5 py-1 rounded-lg gradient-premium text-primary-foreground text-[10px] font-bold flex items-center gap-1">
    ...
    >
      <div className="pt-3 space-y-3">
        ...
        <div className="flex gap-1.5">
          {(Object.keys(DEPTH_LABELS) as Depth[]).map(d => (
            <button key={d} onClick={() => setDepth(d)}
              className={`flex-1 py-2 rounded-xl text-[11px] font-semibold transition-all tap-card ${
                depth === d ? 'gradient-organic text-primary-foreground' : 'glass text-muted-foreground'
              }`}>
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
          className="glass-glow rounded-2xl p-3.5 flex items-center gap-3">
          <div className="relative w-12 h-12 shrink-0">
            <svg viewBox="0 0 48 48" className="w-full h-full -rotate-90">
              <circle cx="24" cy="24" r="20" fill="none" stroke="hsl(var(--muted))" strokeWidth="3.5" />
              <circle cx="24" cy="24" r="20" fill="none" stroke="hsl(var(--primary))" strokeWidth="3.5" strokeLinecap="round"
                strokeDasharray={2 * Math.PI * 20}
                strokeDashoffset={2 * Math.PI * 20 * (1 - completionRate / 100)}
                className="transition-all duration-500" />
            </svg>
            <div className="absolute inset-0 flex items-center justify-center">
              <span className="text-xs font-display font-bold">{completionRate}%</span>
            </div>
          </div>
          <div>
            <p className="text-sm font-semibold">{completed.size} из {protocols.length}</p>
            ...
            return (
              <motion.div key={phase} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
                <button onClick={() => setExpandedPhase(isExpanded ? null : phase)}
                  className={`w-full glass-strong rounded-2xl p-3.5 flex items-center gap-3 tap-card ${isExpanded ? 'rounded-b-none' : ''}`}>
                  <div className={`w-9 h-9 rounded-xl bg-gradient-to-br ${config.gradient} flex items-center justify-center`}>
                    <PhaseIcon className="w-4 h-4 text-foreground/70" />
                  </div>
                  <div className="flex-1 text-left">
                    <p className="text-sm font-semibold">{config.label}</p>
                    <p className="text-[10px] text-muted-foreground">{phaseCompleted}/{phaseItems.length}</p>
                  </div>
                          ...
                          return (
                            <div key={item.id} className="space-y-0.5">
                              <button onClick={() => toggleComplete(item.id)}
                                className={`w-full flex items-start gap-2.5 p-2.5 rounded-xl text-left tap-card ${isDone ? 'opacity-40' : ''}`}>
                                {isDone ? <CheckCircle2 className="w-5 h-5 text-safe shrink-0" /> : <Circle className="w-5 h-5 text-muted-foreground/40 shrink-0" />}
                              ...
                              {item.newsSignal && !isDone && (
                                <div className="ml-7 mr-2 mb-0.5 flex items-start gap-1.5 px-2.5 py-1.5 rounded-lg bg-accent/5">
        ...
        {completed.size > 0 && (
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
            className="glass-glow rounded-2xl p-3.5">
            <p className="text-[10px] uppercase tracking-[0.2em] text-muted-foreground font-semibold mb-1.5">Контроль дня</p>
            <p className="text-xs text-foreground/80 leading-relaxed">
              {completionRate >= 80 ? '🔥 Отличный день — протокол почти выполнен!'
                : completionRate >= 50 ? '💪 Хороший прогресс. Завершите вечерний блок.'
